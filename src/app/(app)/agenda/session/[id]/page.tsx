  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <div className="flex items-start justify-between flex-wrap gap-2">
          <div>
            ...
            <p className="text-sm text-slate-500">Entrenador: {cls.trainer?.name ?? "Sin asignar"}</p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-semibold text-slate-800">
              {booked.length}/{cls.capacity}
            </div>
            <div className="text-xs text-slate-400">plazas ocupadas</div>
            <Link
              href={`/brief/${cls.id}`}
              className="inline-block mt-2 text-xs rounded-lg bg-indigo-600 text-white px-3 py-1.5 hover:bg-indigo-700"
            >
              Abrir Session Brief →
            </Link>
          </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            ...
            {booked.map((b) => (
              <tr key={b.id} className="border-t border-slate-100">
                <td className="px-4 py-2">
                  <Link href={`/members/${b.member.id}`} className="text-indigo-700 hover:underline">
                ...
                <td className="px-4 py-2 text-slate-600">{STATUS_LABEL[b.status]}</td>
                <td className="px-4 py-2">
        ...
        {waitlisted.length > 0 && (
          <>
            <div className="px-5 py-3 border-t border-b border-slate-100 text-sm font-semibold text-slate-700">
