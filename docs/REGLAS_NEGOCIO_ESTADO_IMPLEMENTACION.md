...
| Bitácora del socio (RB-LEAD-008 aplicado a `Member`): destacar y archivar | ✅ | `MemberNote.important` / `MemberNote.archivedAt` (`prisma/schema.prisma`); reparto puro en `src/lib/member-notes.ts`; bloque "Antes de la sesión" en la cabecera de la ficha (`src/app/(app)/members/[id]/note-highlights.tsx`); tests `src/lib/member-notes.test.ts` | 📋 la bitácora se ve con los permisos generales de la ficha, **sin** el gateo de RB-PERFIL-004: el copy del composer y del bloque destacado dice explícitamente que un diagnóstico o algo persistente va a Salud como Lesión (`HealthRecord`) |
