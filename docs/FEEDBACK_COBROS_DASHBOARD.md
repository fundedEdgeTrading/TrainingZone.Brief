...
**`RB-FB-101`** — El **debrief** (`SessionDebrief`) es feedback **del entrenador hacia la sesión**: cómo ha ido cada asistente (esfuerzo/sensación), capturado en <20 s desde el Session Brief. Es visible para el equipo y alimenta el reporte semanal (`RB-RRHH-010`) y las señales de estancamiento (`RB-IA-007`). *(También alimentaba el motor de ofertas, `RB-RRHH-013`, retirado el 23-08-2026.)* **No** es una valoración del entrenador ni un dato confidencial.
